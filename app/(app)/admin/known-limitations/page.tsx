...
const LIMITATIONS: Limitation[] = [
  ...
  },
  {
    ...
    roadmap: 'Current version is deliberately a system-of-intelligence that sits on top of existing tools. Live integrations (Jira webhook / REST sync, ServiceNow CR import) are planned after pilot validation confirms which systems matter most.',
    priority: 'medium',
  },
  {
    id: 6,
    ...
    roadmap: 'In-app first was a deliberate sequencing choice — outbound automation is added once the reminder rules themselves have been pressure-tested against real pilot usage, so we don’t spam users with rules that still need tuning.',
    priority: 'medium',
  },
  {
    id: 7,
    ...
    roadmap: 'The curated presets were built from the filter combinations pilot users actually need on day one. User-defined, saved-to-database views are a natural next step once real usage shows which custom combinations recur most.',
    priority: 'later',
  },
  {
    id: 8,
  ...
  },
  {
    ...
    roadmap: 'The pilot’s first milestone is validating that real enterprise data (initiative lists, ownership hierarchies, benefit claims) maps cleanly onto this schema — seed data was built to mirror that exact shape so the transition is low-risk.',
    priority: 'medium',
  },
  {
    id: 10,
    ...
    roadmap: 'Acceptable for a single-pilot deployment where names are unique and stable. Production hardening should move these to ID-based ownership mapping to eliminate any ambiguity from name changes or duplicates.',
    priority: 'medium',
  },
  {
    id: 11,
