...
import type { BusinessValidation } from '@/lib/types';
...
export default function ValidatePage({ params }: { params: { id: string } }) {
  ...
  const { getItem, saveValidation } = useStore();
  ...
  if (!item) {
    return (
      <div className="py-20 text-center text-sm text-slate-500">
        Item not found.{' '}
        <Link href="/business" className="font-medium text-brand-600 hover:underline">
          Go back
        </Link>
      </div>
    );
      ...
      {saved ? (
        ...
        <form onSubmit={handleSave} className="space-y-5 rounded-xl border border-slate-200 bg-white p-5 shadow-card">
          ...
          <div className="flex gap-3 pt-1">
            <button
              type="submit"
              className="rounded-lg bg-brand-600 px-6 py-2.5 text-sm font-medium text-white shadow-sm transition-colors hover:bg-brand-700"
            >
              Submit Validation
            </button>
            <Link
